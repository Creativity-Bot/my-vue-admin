<template>
    <div>
        <el-card>
            <el-radio-group v-model="radio" size="large" class="mt">
        ...
        <el-card v-for="item in alarmList" :key="item.equNo" class="mt">
            ...
            <el-descriptions :column="4" border direction="vertical" class="mt">
                <el-descriptions-item v-for="(value, key) in item" :key="key" :label="getFieldLabel(key)">
                    <el-tag v-if="key === 'level'" :type="value == 1 ? 'danger': (value==2 ? 'warning': 'info')" size="large">
                ...
                <el-descriptions-item label="操作">
                    <el-button :type="item.status == 2 ? 'warning' : 'primary'" size="large">
